...
void TFile::Skip(std::shared_ptr<Tesses::Framework::Streams::Stream> stream,
                 size_t len) {
    if (stream->CanSeek()) {
        stream->Seek((int64_t)len,
...
void TFile::Ensure(std::shared_ptr<Tesses::Framework::Streams::Stream> stream,
...
uint32_t
TFile::EnsureInt(std::shared_ptr<Tesses::Framework::Streams::Stream> stream) {
    uint8_t buffer[4];
...
TFile::GetString(std::shared_ptr<Tesses::Framework::Streams::Stream> stream) {
    ...
    if (index >= this->strings.size())
        throw VMException(
            "String does not exist in TCrossVM file, expected string index: " +
            std::to_string(index) +
            ", total strings: " + std::to_string(this->strings.size()));
...
TDictionary *TFile::MetadataDecode(GCList &ls, size_t midx) {
    if (midx >= this->metadata.size())
        return nullptr;
    if (this->metadata[midx].second.empty())
        return nullptr;
    if (this->metadata[midx].second[0] != 8)
        return nullptr;
    size_t index = 0;
    ...
    if (GetObjectHeap(ent, dict_res))
        return dict_res;
    return nullptr;
}

void TFile::Load(std::shared_ptr<GC> gc,
                 std::shared_ptr<Tesses::Framework::Streams::Stream> stream) {
    ...
    for (size_t i = 0; i < _len; i++) {
        ...
        } else if (strncmp(table_name, "META", 4) == 0) // structured metadata
        {
            if (tableLen > 4) {
                ...
                auto &data = this->metadata.emplace_back(
                    name, std::vector<uint8_t>(tableLen - 4));
                Ensure(stream, data.second.data(), tableLen - 4);
            } else
                throw VMException("meta tag is not valid");

        } else {
            auto &data = this->sections.emplace_back(
                std::string(table_name, 4), std::vector<uint8_t>(tableLen));
