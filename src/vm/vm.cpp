...
bool InterperterThread::PushPrivateExpression(std::shared_ptr<GC> gc) {

    std::vector<CallStackEntry *> &cse = this->call_stack_entries;
    auto fn = cse.back();
    auto re = fn->env->GetRootEnvironment();
    if (re) {
        fn->Push(gc, re->GetPrivateFromFile(gc, fn->callable->file));
    } else {
        throw VMException("No root environment for private expression");
    }
    return false;
}
...
bool InterperterThread::Divide(std::shared_ptr<GC> gc) {
    ...
    if (std::holds_alternative<std::string>(left) &&
        ...
        cse.back()->Push(gc, Tesses::Framework::Filesystem::VFSPath(
...
bool InterperterThread::Mod(std::shared_ptr<GC> gc) {
    ...
    if (std::holds_alternative<int64_t>(left) &&
        ...
        cse.back()->Push(gc,
               ...
               std::holds_alternative<int64_t>(right)) {
        cse.back()->Push(
            gc, fmod(std::get<double>(left), std::get<int64_t>(right)));
    } else if (std::holds_alternative<int64_t>(left) &&
               std::holds_alternative<double>(right)) {
        cse.back()->Push(
...
bool InterperterThread::Lt(std::shared_ptr<GC> gc) {
    ...
    if (std::holds_alternative<int64_t>(left) &&
        ...
        cse.back()->Push(gc, r < 0);
               ...
               std::holds_alternative<std::string>(right)) {
        cse.back()->Push(gc, std::get<std::string>(left) <
                                 std::get<std::string>(right));
...
bool InterperterThread::Lte(std::shared_ptr<GC> gc) {
    ...
    if (std::holds_alternative<int64_t>(left) &&
        ...
        cse.back()->Push(gc, std::get<double>(left) <= std::get<double>(right));
               ...
               std::holds_alternative<int64_t>(right)) {
        cse.back()->Push(gc,
                         std::get<double>(left) <= std::get<int64_t>(right));
    } else if (std::holds_alternative<int64_t>(left) &&
               std::holds_alternative<double>(right)) {
        cse.back()->Push(gc,
                         std::get<int64_t>(left) <= std::get<double>(right));
    } else if (std::holds_alternative<char>(left) &&
               std::holds_alternative<char>(right)) {
        cse.back()->Push(gc, std::get<char>(left) <= std::get<char>(right));
    } else if (std::holds_alternative<char>(left) &&
        ...
        if (cls != nullptr) {
            ...
            cse.back()->Push(gc,
                             dynDict->CallMethod(ls, "operator<=", {right}));
            return false;
        }
    ...
    if (!cse.empty()) {
        ...
        if (std::holds_alternative<int64_t>(cnt)) {
            ...
            if (std::holds_alternative<std::string>(key)) {
                return ExecuteMethod2(gc, instance, std::get<std::string>(key),
                                      args);
            }
            stk->Push(gc, Undefined());
            return false;
...
bool InterperterThread::SetVariable(std::shared_ptr<GC> gc) {
    ...
    if (!cse.empty()) {
        ...
        if (std::holds_alternative<std::string>(key)) {
            ...
            stk->Push(gc, stk->env->SetVariable(ls, std::get<std::string>(key),
                                                value));
...
bool InterperterThread::DeclareVariable(std::shared_ptr<GC> gc) {
    ...
    if (!cse.empty()) {
        ...
        if (std::holds_alternative<std::string>(key)) {
            ...
            throw VMException("[DECLAREVARIABLE] Can't pop string, got type " +
                              GetObjectTypeString(key) + " = " +
                              ToString(gc, key) + ".");
        }
    }
    return false;
...
bool InterperterThread::JumpIfDefined(std::shared_ptr<GC> gc) {
    ...
    if (stk->ip + 4 <= stk->callable->closure->code.size()) {
        uint32_t n =
            BitConverter::ToUint32BE(stk->callable->closure->code[stk->ip]);
...
bool InterperterThread::LineInfo(std::shared_ptr<GC> gc) {
    ...
    // TODO: implement lines (this will make the language work until we get it
    // rigged up)
...
bool InterperterThread::Nop(std::shared_ptr<GC> gc) { return false; }
bool InterperterThread::AppendList(std::shared_ptr<GC> gc) {
    ...
    if (std::holds_alternative<THeapObjectHolder>(objhold)) {
        ...
        if(dict != nullptr)
        {
            ...
            {
                dict->SetValue(std::get<std::string>(potential_str),
        stk->Pop(ls));
            }
        }*/
    }
...
bool InterperterThread::AppendDictionary(std::shared_ptr<GC> gc) {
    ...
    if (std::holds_alternative<THeapObjectHolder>(objhold) &&
        ...
        if (dict != nullptr) {
            ...
            auto obj = cls->GetValue(cse.back()->callable->className,
                                     "set" + std::get<std::string>(k));
            TCallable *callable;
            if (GetObjectHeap(obj, callable)) {
                ...
                cls->SetValue(cse.back()->callable->className,
                              std::get<std::string>(k), value);
            }
        }
    }
...
bool InterperterThread::PushClosure(std::shared_ptr<GC> gc) {
    ...
    if (!cse.empty()) {
        ...
        if (stk->ip + 4 <= code.size()) {
            ...
            GCList ls(gc);
            TClosure *closure =
                TClosure::Create(ls, stk->env, stk->callable->file, n, true);
...
bool InterperterThread::PushScopelessClosure(std::shared_ptr<GC> gc) {
    ...
    if (!cse.empty()) {
        ...
        if (stk->ip + 4 <= code.size()) {
            ...
            GCList ls(gc);
            TClosure *closure =
                TClosure::Create(ls, stk->env, stk->callable->file, n, false);
    ...
    if (stk->ip + 4 <= code.size()) {
        ...
        for (uint32_t i = 0; i < n; i++) {
            if (!stk->env->defers.empty()) {
                ls.Add(stk->env);
                callable.insert(callable.end(), stk->env->defers.begin(),
                                stk->env->defers.end());
            }
            stk->scopes--;
    ...
    if (!cse.empty()) {
        ...
        try {
            while (stk->ip < 0xFFFFFFFF &&
                   stk->ip < stk->callable->closure->code.size()) {
...

            {
                ...
                while (!cse.empty()) {
                    ...
                    for (uint32_t i = 0; i < r->scopes; i++) {
                        if (!e->defers.empty()) {
                            ls.Add(e);
                            callable.insert(callable.end(), e->defers.begin(),
                                            e->defers.end());
                        }
...
void InterperterThread::AddCallStackEntry(GCList &ls, TClosure *closure,
    ...
    cse->env = closure->chunkId == 0 ? closure->env
               : closure->ownScope   ? closure->env->GetSubEnvironment(ls)
                                     : closure->env;
    cse->ip = 0;
    if (closure->closure->args.empty() && closure->chunkId != 0) {
        ...
        if (args.size() < required) {
            throw VMException("Called a function that expected at least " +
                              std::to_string(required) + " args but got " +
