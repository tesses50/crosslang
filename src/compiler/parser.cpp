...
SyntaxNode TObject2SyntaxNode(TObject o) {
    ...
    if (std::holds_alternative<double>(o))
        return std::get<double>(o);
    if (std::holds_alternative<char>(o))
        return std::get<char>(o);
    ...
    if (GetObjectHeap(o, list)) {
        if (list->Count() == 0)
            return AdvancedSyntaxNode::Create(ArrayExpression, true, {});

        SyntaxNode node = TObject2SyntaxNode(list->Get(0));

        for (int64_t i = 1; i < list->Count(); i++)
            node = AdvancedSyntaxNode::Create(
                CommaExpression, true,
                {node, TObject2SyntaxNode(list->Get(i))});
...
bool Parser::IsAnyIdentifier(std::initializer_list<std::string> idents,
                             bool pop) {
    if (i < tokens.size()) {
...
static std::string EnsureSafeVariable(LexToken token) {
    if (token.text.find("__compGen") == 0) {
        throw SyntaxException(token.lineInfo,
                              "__compGen* is reserved for compilers (this "
                              "error is enforced by the compiler)");
    }
    return token.text;
}
    ...
    if (i < tokens.size()) {
        if (tokens[i].type != LexTokenType::Symbol) {
            throw SyntaxException(
                tokens[i].lineInfo,
                "expected the symbol \"" + txt + "\" but got the " +
...
Parser::Parser(std::vector<LexToken> tokens)
    : Parser(tokens, nullptr, nullptr) {}
Parser::Parser(std::vector<LexToken> tokens, std::shared_ptr<GC> gc,
...
void Parser::ParseHtml(std::vector<SyntaxNode> &nodes, std::string var) {
    if (this->IsSymbol("!", true)) {
        if (this->i < this->tokens.size() &&
            this->tokens[this->i].type == LexTokenType::Identifier) {
            ...
            if (identifier == "DOCTYPE") {
                if (this->i < this->tokens.size() &&
                    ...
                    nodes.push_back(AdvancedSyntaxNode::Create(
        ...
        auto parseFn = [this, var](std::vector<SyntaxNode> &nodes,
                ...
                if (this->IsSymbol("<", false)) {
                    nodes.push_back(AdvancedSyntaxNode::Create(
                        ...
                        {AdvancedSyntaxNode::Create(
                            ...
                            {AdvancedSyntaxNode::Create(GetVariableExpression,
                                                        true, {var}),
                             Tesses::Framework::Http::HttpUtils::HtmlEncode(
                                 this->tokens[i].whiteSpaceCharsBefore)})}));

                    this->i++;

                    if (this->IsSymbol("/")) {
                        if (this->IsIdentifier("if", false) &&
                            tagName == "if") {
                            this->i -= 2;
                            break;
                        }
                        ...
                        this->EnsureSymbol(">");
                        if (tagName != "if" && tagName != "true" &&
                            tagName != "false" && tagName != "for" &&
                            tagName != "while" && tagName != "do" &&
                            tagName != "each" && tagName != "null" &&
                            tagName != "return") {
                            std::string myVal = "</";
                            myVal += tagName;
                            myVal += ">";
                            nodes.push_back(AdvancedSyntaxNode::Create(
                                ...
                                {AdvancedSyntaxNode::Create(
                                    AddExpression, true,
                                    {AdvancedSyntaxNode::Create(
                                         GetVariableExpression, true, {var}),
                                     myVal})}));
                        }
                        break;
                    } else {
                    ...
                    while (this->i < this->tokens.size() &&
                           !this->IsSymbol("{", false) &&
                           !this->IsSymbol("<", false)) {
                        str += this->tokens[this->i].whiteSpaceCharsBefore +
                               this->tokens[this->i].text;
                        this->i++;
                    }
            ...
            if (tagName == "raw") {
                ...
                while (this->IsSymbol("<")) {
                    if (this->IsSymbol("/")) {
                        ...
                        parseFn(_nodes, "true");
                        truth = AdvancedSyntaxNode::Create(ScopeNode, false,
                                                           _nodes);
                        ...
                        falsey = AdvancedSyntaxNode::Create(ScopeNode, false,
                                                            _nodes);
                    }
                }
                nodes.push_back(AdvancedSyntaxNode::Create(
                    IfStatement, false, {expr, truth, falsey}));
            } else if (tagName == "null") {
                EnsureSymbol(">");
                std::vector<SyntaxNode> _nodes;
                parseFn(_nodes, "null");
                nodes.push_back(
                    AdvancedSyntaxNode::Create(ScopeNode, false, _nodes));
                ...
                nodes.push_back(AdvancedSyntaxNode::Create(
                    WhileStatement, false, {expr, body}));
                ...
                nodes.push_back(AdvancedSyntaxNode::Create(DoStatement, false,
                                                           {expr, body}));
            } else if (tagName == "for") {
                ...
                nodes.push_back(AdvancedSyntaxNode::Create(
                    ForStatement, false, {init, cond, inc, body}));
                ...
                nodes.push_back(AdvancedSyntaxNode::Create(EachStatement, false,
                                                           {item, list, body}));
            }
                    ...
                    if (this->i < this->tokens.size() &&
                        ...
                        if (this->IsSymbol(">")) {
                            ...
                            if (std::holds_alternative<AdvancedSyntaxNode>(
                                ...
                                myVal +=
                                    std::to_string(std::get<double>(expr)) +
                                    "\"";
                                ...
                                myVal +=
                                    std::to_string(std::get<int64_t>(expr)) +
                                    "\"";
                                ...
                                myVal += std::holds_alternative<bool>(expr)
                                             ? "true\""
                                             : "false\"";
                                    ...
                                    {AdvancedSyntaxNode::Create(
                                        ...
                                        {AdvancedSyntaxNode::Create(
                                         ...
                                         myVal})}));
                                           ...
                                           expr)) {
                                myVal += Tesses::Framework::Http::HttpUtils::
                                             HtmlEncode(
                                                 std::get<std::string>(expr)) +
                                         "\"";
                                nodes.push_back(AdvancedSyntaxNode::Create(
                                    CompoundAssignExpression, true,
                                    {AdvancedSyntaxNode::Create(
                ...
                if (tagName != "img" && tagName != "input" && tagName != "br" &&
                    tagName != "hr" && tagName != "link" && tagName != "meta") {
...
uint32_t Parser::NewId() { return id++; }
SyntaxNode Parser::ParseValue() {
    ...
    if (tokens[i].type == LexTokenType::String) {
    ...
    } else if (tokens[i].type == LexTokenType::Symbol &&
               tokens[i].text == ".") {
        i++;
        if (IsSymbol("[")) {
            node = AdvancedSyntaxNode::Create(GetVariableExpression, true,
                                              {ParseExpression()});
            EnsureSymbol("]");
        } else {
        ...
        std::vector<SyntaxNode> syntaxNode = {compHtml};
        ...
        if (variable.type == LexTokenType::Symbol && variable.text == ".") {
            ...
            node = AdvancedSyntaxNode::Create(
                ...
                {AdvancedSyntaxNode::Create(GetVariableExpression, true,
                                            {ParseExpression()})});
            EnsureSymbol("]");
        } else if (variable.type == LexTokenType::Symbol &&
                   variable.text == "[") {
            node = AdvancedSyntaxNode::Create(
                DeclareExpression, true,
                {AdvancedSyntaxNode::Create(ArrayExpression, true,
                                            {ParseExpression()})});
            EnsureSymbol("]");
        } else if (variable.type != LexTokenType::Identifier)
            throw SyntaxException(variable.lineInfo,
                                  "Expected an identifier got a " +
                                      LexTokenType_ToString(variable.type) +
                                      " \"" + variable.text + "\"");
        else {
            node = AdvancedSyntaxNode::Create(DeclareExpression, true,
                                              {EnsureSafeVariable(variable)});
        }
        ...
        if (IsSymbol("{", false)) {
            ...
            n = AdvancedSyntaxNode::Create(ReturnStatement, false,
                                           {ParseExpression()});
        }
        ...
        auto variable = tokens[i];
        i++;
        if (variable.type != LexTokenType::Identifier &&
            variable.type != LexTokenType::Symbol)
            throw SyntaxException(variable.lineInfo,
                                  "Expected an identifier or a symbol got a " +
                                      LexTokenType_ToString(variable.type) +
                                      " \"" + variable.text + "\"");
        node = AdvancedSyntaxNode::Create(GetVariableExpression, true,
                                          {"operator" + variable.text});
        ...
        node = AdvancedSyntaxNode::Create(
            GetFieldExpression, true,
            {AdvancedSyntaxNode::Create(GetVariableExpression, true, {"New"}),
             variable.text});
        ...
        auto embed = tokens[i];
        i++;
        if (embed.type != LexTokenType::String)
            throw SyntaxException(embed.lineInfo,
                                  "Expected an string for embed got a " +
                                      LexTokenType_ToString(embed.type) +
                                      " \"" + embed.text + "\"");
        EnsureSymbol(")");
        ...
        if (embed.type != LexTokenType::String)
            throw SyntaxException(embed.lineInfo,
                                  "Expected an string for embed got a " +
                                      ...
                                      " \"" + embed.text + "\"");
        EnsureSymbol(")");
        node = AdvancedSyntaxNode::Create(EmbedStreamExpression, true,
                                          {embed.text});
        ...
        if (embed.type != LexTokenType::String)
            throw SyntaxException(embed.lineInfo,
                                  "Expected an string for embed got a " +
                                      ...
                                      " \"" + embed.text + "\"");
        EnsureSymbol(")");
        node = AdvancedSyntaxNode::Create(EmbedDirectoryExpression, true,
                                          {embed.text});
    } else if (IsIdentifier("private")) {
        node = AdvancedSyntaxNode::Create(GetPrivateExpression, true, {});
        ...
        if (token.text.size() == 1 && token.text[0] == '0') {
            lngNum = 0;
        } else if (token.text.size() > 0 && token.text[0] == '0') {
        ...
        if (hasNumber && this->IsSymbol(".", false) && i + 1 < tokens.size() &&
            tokens[i + 1].type == LexTokenType::Identifier) {
    ...
    if (IsSymbol("=>")) {
        if (IsSymbol("{", false)) {
            node = AdvancedSyntaxNode::Create(ClosureExpression, true,
                                              {node, ParseNode()});
        } else {
            node = AdvancedSyntaxNode::Create(
                ClosureExpression, true,
                {node, AdvancedSyntaxNode::Create(ReturnStatement, false,
                                                  {ParseExpression()})});
        }
    }
    while (IsAnySymbol({".", "[", "("})) {
        if (tkn.text == ".") {
            ...
            if (IsSymbol("[")) {
                ...
                node = AdvancedSyntaxNode::Create(GetFieldExpression, true,
                                                  {node, ParseExpression()});
                EnsureSymbol("]");
                continue;
            }
            if (tokens[i].type != LexTokenType::Identifier)
                throw std::runtime_error("Not an identifier (member)");
            std::string name = tokens[i].text;
            if (name == "operator") {
                if (i >= tokens.size())
                    throw std::out_of_range("End of file");
                auto op = tokens[i];
                if (op.type != LexTokenType::Identifier &&
                    op.type != LexTokenType::Symbol)
                    throw SyntaxException(
                        op.lineInfo,
                        "Expected an identifier or a symbol got a " +
                            LexTokenType_ToString(op.type) + " \"" + op.text +
                            "\"");
            ...
            node = AdvancedSyntaxNode::Create(GetArrayExpression, true,
                                              {node, ParseExpression()});
            EnsureSymbol("]");
        } else if (tkn.text == "(") {
            if (IsSymbol(")", false)) {
                node = AdvancedSyntaxNode::Create(FunctionCallExpression, true,
                                                  {node});
            } else {
                node = AdvancedSyntaxNode::Create(FunctionCallExpression, true,
                                                  {node, ParseExpression()});
    ...
    if (IsSymbol("++")) {
        node = AdvancedSyntaxNode::Create(PostfixIncrementExpression, true,
                                          {node});
    } else if (IsSymbol("--")) {
        node = AdvancedSyntaxNode::Create(PostfixDecrementExpression, true,
                                          {node});
    }
...
SyntaxNode Parser::ParseUnary() {
    if (IsSymbol("-")) {
        return AdvancedSyntaxNode::Create(NegativeExpression, true,
                                          {ParseUnary()});
    } else if (IsSymbol("!")) {
        return AdvancedSyntaxNode::Create(NotExpression, true, {ParseUnary()});
    } else if (IsSymbol("~")) {
        return AdvancedSyntaxNode::Create(BitwiseNotExpression, true,
                                          {ParseUnary()});
    } else if (IsSymbol("++")) {
        return AdvancedSyntaxNode::Create(PrefixIncrementExpression, true,
                                          {ParseUnary()});
    } else if (IsSymbol("--")) {
        return AdvancedSyntaxNode::Create(PrefixDecrementExpression, true,
                                          {ParseUnary()});
        ...
        if (std::holds_alternative<AdvancedSyntaxNode>(v)) {
            ...
            if (asn.nodeName != ClosureExpression)
                throw SyntaxException(tkn.lineInfo,
                                      "async must be used only with a closure");
            if (asn.nodes.size() != 2)
            ...
            throw SyntaxException(
                tkn.lineInfo,
                "async must be used only with a closure, not a simple value");
        }
        ...
        if (std::holds_alternative<AdvancedSyntaxNode>(v)) {
            ...
            if (asn.nodeName != ClosureExpression)
                throw SyntaxException(
                    tkn.lineInfo,
                    "enumerable must be used only with a closure");
            if (asn.nodes.size() != 2)
                throw SyntaxException(tkn.lineInfo, "invalid closure");
            ...
            throw SyntaxException(tkn.lineInfo,
                                  "enumerable must be used only with a "
                                  "closure, not a simple value");
        }
    } else if (IsSymbol("/")) {
        if (this->i < this->tokens.size() &&
            (this->tokens[this->i].type == LexTokenType::String ||
             this->tokens[this->i].type == LexTokenType::Identifier)) {
            return AdvancedSyntaxNode::Create(
                DivideExpression, true,
...
SyntaxNode Parser::ParseFactor() {
    ...
    while (IsAnySymbol({"*", "/", "%"}, true)) {
        if (tkn.text == "*") {
            expr = AdvancedSyntaxNode::Create(TimesExpression, true,
                                              {expr, ParseUnary()});
        } else if (tkn.text == "/") {
            expr = AdvancedSyntaxNode::Create(DivideExpression, true,
                                              {expr, ParseUnary()});
        } else if (tkn.text == "%") {
            expr = AdvancedSyntaxNode::Create(ModExpression, true,
                                              {expr, ParseUnary()});
        }
    }
    return expr;
...
SyntaxNode Parser::ParseSum() {
    ...
    while (IsAnySymbol({"+", "-"}, true)) {
        if (tkn.text == "+") {
            expr = AdvancedSyntaxNode::Create(AddExpression, true,
                                              {expr, ParseFactor()});
        } else if (tkn.text == "-") {
            expr = AdvancedSyntaxNode::Create(SubExpression, true,
                                              {expr, ParseFactor()});
        }
    }
    return expr;
...
SyntaxNode Parser::ParseAssignment() {
    ...
    if (IsSymbol("=")) {
        return AdvancedSyntaxNode::Create(AssignExpression, true,
        ...
        return AdvancedSyntaxNode::Create(
            CompoundAssignExpression, true,
            {AdvancedSyntaxNode::Create(TimesExpression, true,
                                        {node, ParseAssignment()})});
    } else if (IsSymbol("/=")) {
        return AdvancedSyntaxNode::Create(
            CompoundAssignExpression, true,
            {AdvancedSyntaxNode::Create(DivideExpression, true,
                                        {node, ParseAssignment()})});
    } else if (IsSymbol("%=")) {
        return AdvancedSyntaxNode::Create(
            CompoundAssignExpression, true,
            {AdvancedSyntaxNode::Create(ModExpression, true,
                                        {node, ParseAssignment()})});
    } else if (IsSymbol("<<=")) {
        return AdvancedSyntaxNode::Create(
            CompoundAssignExpression, true,
            {AdvancedSyntaxNode::Create(LeftShiftExpression, true,
                                        {node, ParseAssignment()})});
    } else if (IsSymbol(">>=")) {
        return AdvancedSyntaxNode::Create(
            CompoundAssignExpression, true,
            {AdvancedSyntaxNode::Create(RightShiftExpression, true,
                                        {node, ParseAssignment()})});
    } else if (IsSymbol("|=")) {
        return AdvancedSyntaxNode::Create(
            CompoundAssignExpression, true,
            {AdvancedSyntaxNode::Create(BitwiseOrExpression, true,
                                        {node, ParseAssignment()})});
    } else if (IsSymbol("&=")) {
        return AdvancedSyntaxNode::Create(
            CompoundAssignExpression, true,
            {AdvancedSyntaxNode::Create(BitwiseAndExpression, true,
                                        {node, ParseAssignment()})});
    } else if (IsSymbol("^=")) {
        return AdvancedSyntaxNode::Create(
            CompoundAssignExpression, true,
            {AdvancedSyntaxNode::Create(XOrExpression, true,
                                        {node, ParseAssignment()})});
    } else if (IsSymbol("?\?=")) {
        return AdvancedSyntaxNode::Create(
            CompoundAssignExpression, true,
            {AdvancedSyntaxNode::Create(NullCoalescingExpression, true,
                                        {node, ParseAssignment()})});
    }
    return node;
}
...
SyntaxNode Parser::ParseNode(bool isRoot) {
    ...
    if (i < tokens.size() && !isRoot && tokens[i].type == Documentation) {
        ...
        i++;
        if (i < tokens.size() && tokens[i].text == "class" &&
            tokens[i].type == LexTokenType::Identifier)
            documentation = txt;
        else
            return AdvancedSyntaxNode::Create(DocumentationStatement, false,
                                              {txt, ParseNode()});
    }
    if (IsSymbol("{") || isRoot) {
        ...
        while (i < tokens.size() && (isRoot || !IsSymbol("}", false))) {
            auto token = tokens[i];
            if (CanEmit(token.lineInfo))
                aSN.nodes.push_back(AdvancedSyntaxNode::Create(
                    LineNode, false,
                    {(int64_t)tokens[i].lineInfo.line,
                     tokens[i].lineInfo.filename}));
    ...
    if (IsIdentifier("for")) {
        ...
        return AdvancedSyntaxNode::Create(ForStatement, false,
                                          {init, cond, inc, body});
    }
    ...
    if (IsIdentifier("class")) {
        if (i < tokens.size()) {
            ...
            if (IsSymbol(":", true)) {
                ...
                name_and_methods.push_back(AdvancedSyntaxNode::Create(
                    GetVariableExpression, true, {"ClassObject"}));
            }
            EnsureSymbol("{");
                ...
                if (i < tokens.size()) {
                    if (IsAnyIdentifier(
                        ...
                        if (IsIdentifier("abstract")) {
                            if (myTkn.text == "static")
                                throw SyntaxException(
                                    myTkn.lineInfo, "Static abstract function "
                                                    "doesn't make sense");
                            auto nameAndArgs = ParseExpression();
                            EnsureSymbol(";");

                            name_and_methods.push_back(
                                AdvancedSyntaxNode::Create(
                                    AbstractMethodStatement, false,
                            ...
                            SyntaxNode field = AdvancedSyntaxNode::Create(
                                FieldStatement, false,
                                {documentation, myTkn.text,
                            ...
                            if (IsIdentifier("async")) {
                                ...
                                if (IsSymbol("{", false)) {
                                    ...
                                    throw SyntaxException(
                                        tokens[i].lineInfo,
                                        "expected the symbol \"{\" on "
                                        "enumerable but got the symbol or "
                                        "other token \"" +
                                            tokens[i].text + "\"");
                                }
                            } else {
                        ...
                        throw SyntaxException(
                            tokens[i].lineInfo,
                            "Need public, private, protected or static");
                    }
                }
            }
            EnsureSymbol("}");
            return AdvancedSyntaxNode::Create(ClassStatement, false,
                                              name_and_methods);
        } else
            throw std::out_of_range("End of file");
    }

    if (IsIdentifier("enumerable", false) && i + 1 < tokens.size() &&
        tokens[i + 1].text == "func" && i + 1 < tokens.size() &&
        tokens[i + 1].type == LexTokenType::Identifier) {
        i += 2;
        auto nameAndArgs = ParseExpression();
        ...
        if (IsSymbol("{", false)) {
            ...
            throw SyntaxException(tokens[i].lineInfo,
                                  "expected the symbol \"{\" on enumerable but "
                                  "got the symbol or other token \"" +
                                      tokens[i].text + "\"");
        }
    }
    if (IsIdentifier("async", false) && i + 1 < tokens.size() &&
        tokens[i + 1].text == "func" &&
        tokens[i + 1].type == LexTokenType::Identifier) {
        i += 2;
        // async func
        auto nameAndArgs = ParseExpression();
                     ...
                     {AdvancedSyntaxNode::Create(
                         ...
                         {AdvancedSyntaxNode::Create(
                          ...
                          AdvancedSyntaxNode::Create(
                              ...
                              {AdvancedSyntaxNode::Create(ParenthesesExpression,
                                                          true, {}),
                               AdvancedSyntaxNode::Create(ReturnStatement,
                                                          false, {v})})})})});
        }
    }
    if (IsIdentifier("meta")) {
        if (i >= tokens.size() || tokens[i].type != LexTokenType::String) {
            std::cout
                << "WARN: meta is a conditional keyword,\nif you suffix it "
                   "with a string, it will be assumed to be a metadata tag"
                << std::endl;
            i--;
        } else {
            std::string name = tokens[i++].text;
            ...
            return AdvancedSyntaxNode::Create(
                MetadataStatement, false,
                {name, AdvancedSyntaxNode::Create(DictionaryExpression, true,
                                                  {expr})});
        }
    }
    if (IsIdentifier("func")) {
        ...
        if (IsSymbol("{", false)) {
            ...
            return AdvancedSyntaxNode::Create(
                FunctionStatement, false,
                {nameAndArgs,
                 AdvancedSyntaxNode::Create(ReturnStatement, false, {v})});
        }
    }
    ...
    if (IsIdentifier("case")) {
        auto r = AdvancedSyntaxNode::Create(CaseStatement, false,
                                            {ParseExpression()});
        EnsureSymbol(":");
        return r;
    }
    ...
    if (IsSymbol("<", false) && this->i + 1 < this->tokens.size() &&
        this->tokens[this->i + 1].type == LexTokenType::Identifier &&
        this->tokens[this->i + 1].text == "return") {
        this->i++;
        uint32_t htmlId = NewId();
        std::string compHtml = "__compGenHtml";
        compHtml.append(std::to_string(htmlId));
        ...
        auto thytoken =
            AdvancedSyntaxNode::Create(HtmlRootExpression, true, syntaxNode);
    ...
    if (IsIdentifier("throw")) {
        ...
        return AdvancedSyntaxNode::Create(
            ...
            {v, tkn2.lineInfo.filename, (int64_t)tkn2.lineInfo.line,
             (int64_t)tkn2.lineInfo.column, (int64_t)tkn.lineInfo.offset});
    }
    ...
    if (IsIdentifier("try")) {
        ...
        return AdvancedSyntaxNode::Create(
            TryStatement, false, {tryBody, catchNode, catchEx, finally});
    }
    if (IsIdentifier("defer")) {
        if (IsSymbol("{", false)) {
            return AdvancedSyntaxNode::Create(DeferStatement, false,
                                              {ParseNode()});
        } else {
            auto v = ParseExpression();
...
SyntaxNode Parser::ParseNullCoalescing() {
    ...
    while (IsSymbol("?\?")) {
        expr = AdvancedSyntaxNode::Create(NullCoalescingExpression, true,
                                          {expr, ParseLOr()});
    }
    return expr;
}
SyntaxNode Parser::ParseTernary() {
    ...
    if (IsSymbol("?")) {
        ...
        return AdvancedSyntaxNode::Create(TernaryExpression, true,
                                          {node, yes, no});
    }
    return node;
}
SyntaxNode Parser::ParseShift() {
    ...
    while (IsAnySymbol({"<<", ">>"}, true)) {
        if (tkn.text == "<<") {
            expr = AdvancedSyntaxNode::Create(LeftShiftExpression, true,
                                              {expr, ParseSum()});
        } else if (tkn.text == ">>") {
            expr = AdvancedSyntaxNode::Create(RightShiftExpression, true,
                                              {expr, ParseSum()});
        }
    }
    return expr;
...
SyntaxNode Parser::ParseRel() {
    ...
    while (IsAnySymbol({"<", ">", "<=", ">="}, true)) {
        if (tkn.text == "<") {
            expr = AdvancedSyntaxNode::Create(LessThanExpression, true,
                                              {expr, ParseShift()});
        } else if (tkn.text == ">") {
            expr = AdvancedSyntaxNode::Create(GreaterThanExpression, true,
                                              {expr, ParseShift()});
        } else if (tkn.text == "<=") {
            expr = AdvancedSyntaxNode::Create(LessThanEqualsExpression, true,
                                              {expr, ParseShift()});
        } else if (tkn.text == ">=") {
            expr = AdvancedSyntaxNode::Create(GreaterThanEqualsExpression, true,
                                              {expr, ParseShift()});
        }
    }
    return expr;
...
SyntaxNode Parser::ParseEq() {
    ...
    while (IsAnySymbol({"==", "!="}, true)) {
        if (tkn.text == "==") {
            expr = AdvancedSyntaxNode::Create(EqualsExpression, true,
                                              {expr, ParseRel()});
        } else if (tkn.text == "!=") {
            expr = AdvancedSyntaxNode::Create(NotEqualsExpression, true,
                                              {expr, ParseRel()});
        }
    }
    return expr;
...
SyntaxNode Parser::ParseBAnd() {
    ...
    while (IsSymbol("&")) {
        expr = AdvancedSyntaxNode::Create(BitwiseAndExpression, true,
                                          {expr, ParseEq()});
    }
    return expr;
}
...
SyntaxNode Parser::ParseXOr() {
    ...
    while (IsSymbol("^")) {
        expr = AdvancedSyntaxNode::Create(XOrExpression, true,
                                          {expr, ParseBAnd()});
    }
    return expr;
}
SyntaxNode Parser::ParseBOr() {
    ...
    while (IsSymbol("|")) {
        expr = AdvancedSyntaxNode::Create(BitwiseOrExpression, true,
                                          {expr, ParseXOr()});
    }
    return expr;
}
SyntaxNode Parser::ParseLAnd() {
    ...
    while (IsSymbol("&&")) {
        expr = AdvancedSyntaxNode::Create(LogicalAndExpression, true,
                                          {expr, ParseBOr()});
    }
    return expr;
}
SyntaxNode Parser::ParseLOr() {
    ...
    while (IsSymbol("||")) {
        expr = AdvancedSyntaxNode::Create(LogicalOrExpression, true,
                                          {expr, ParseLAnd()});
    }
    return expr;
}
