...
static std::shared_ptr<SMTPBody>
...
class DummyTNativeObjectThatReturnsHttpDictionary
    : public TNativeObjectThatReturnsHttpDictionary {
    bool isAvail;

  public:
    DummyTNativeObjectThatReturnsHttpDictionary() { this->isAvail = true; }
    bool IsAvailable() { return this->isAvail; }
    void Close() { this->isAvail = false; }
    std::string TypeName() { return "Dummy"; }
...
class THttpDictionary : public TNativeObject {
  ...
  public:
    THttpDictionary(HttpDictionary *dict,
                    TNativeObjectThatReturnsHttpDictionary *ctx) {
        this->dict = dict;
        this->ctx = ctx;
    }
    std::string TypeName() { return "Net.Http.HttpDictionary"; }
    void Mark() {
        if (this->marked)
            return;
        this->marked = true;
    ...
    TObject CallMethod(GCList &ls, std::string key, std::vector<TObject> args) {
        if (!ctx->IsAvailable())
            ...
            double value;
            if (GetArgument(args, 0, key) &&
                dict->TryGetFirstDouble(key, value)) {
                return value;
            }
            return nullptr;
            ...
            if (GetArgument(args, 0, key) &&
                dict->TryGetFirstDate(key, value)) {
            ...
            if (this->dict->TryGetFirst("Content-Disposition", cdStr) &&
                ContentDisposition::TryParse(cdStr, cd)) {
...
class TServerContext : public TNativeObjectThatReturnsHttpDictionary {
  ...
  public:
    TServerContext(ServerContext *ctx) { this->ctx = ctx; }
    std::string TypeName() { return "Net.Http.ServerContext"; }
    bool IsAvailable() { return ctx != nullptr; }
    ServerContext *GetContext() { return this->ctx; }
    TObject CallMethod(GCList &ls, std::string key, std::vector<TObject> args) {
        if (this->ctx == nullptr)
            return Undefined();
        else if (key == "getEncrypted")
            return ctx->encrypted;
        else if (key == "getMethod")
            return ctx->method;
        else if (key == "getIP")
            return ctx->ip;
        else if (key == "getPort")
            ...
            return TNativeObject::Create<THttpDictionary>(
                ls, &ctx->responseHeaders, this);
        else if (key == "getPathArguments")
            return TNativeObject::Create<THttpDictionary>(
                ls, &ctx->pathArguments, this);
        else if (key == "GetStream")
            return ctx->GetStream();
        else if (key == "OpenRequestStream")
            return ctx->OpenRequestStream();
        else if (key == "OpenResponseStream")
            return ctx->OpenResponseStream();
            ...
            if (GetArgumentHeap(args, 0, callable)) {
                ctx->ParseFormData(
                    [callable, &ls](std::string a, std::string b, std::string c)
                ...
                ctx->ParseFormData(
                    [vfs, &response, &ls, &i](std::string mime,
                                              std::string filename,
                                              std::string name)
                        -> std::shared_ptr<Tesses::Framework::Streams::Stream> {
                        std::string realFileName =
                            "/" + std::to_string(i) + ".bin";
                        i++;
                        response.push_back(TDictionary::Create(
                            ls, {TDItem("RealFileName", realFileName),
                                 TDItem("FileName", filename),
                                 TDItem("Mime", mime), TDItem("Name", name)}));
            ...
            if (args.size() > 0) {
                ctx->WithMimeType("application/json")
                    .SendText(Json_Encode(args[0]));
            }
            ...
            if (!args.empty() &&
                std::holds_alternative<
                    ...
                    args[0])) {
                ctx->SendServerSentEvents(
                    std::get<std::shared_ptr<
                        Tesses::Framework::Http::ServerSentEvents>>(args[0]));
            }
            ...
            if (GetArgumentHeap(args, 0, callable)) {
                ...
                ctx->WithHeaderIntercepter(
                    [marked](ServerContext &ctx) -> bool {
                        ...
                        if (GetObjectHeap(obj, callable)) {
                            ...
                            auto ptr =
                                TNativeObject::Create<TServerContext>(ls, &ctx);
            ...
            if (GetArgumentHeap(args, 0, dict)) {
                ...
                ctx->StartWebSocketSession(
                    [dict,
                     &ls](std::function<void(WebSocketMessage &)> sendMessage,
                          std::function<void()> ping,
                          std::function<void()> close) -> void {
                        GCList ls2(ls.GetGC());
                        dict->CallMethod(
                            ls2, "Open",
                            {TExternalMethod::Create(
                                 ls2, "Send a message",
                                 {"messageTextOrByteArray"},
                                 [sendMessage](
                                     GCList &ls,
                                     ...
                                     if (GetArgument(args, 0, str)) {
                                         ...
                                         sendMessage(msg);
                                     } else if (GetArgumentHeap(args, 0,
                                                                bArray)) {
...
class TDictionaryHttpRequestBody : public HttpRequestBody {
  ...
  public:
    ...
    void HandleHeaders(HttpDictionary &dict) {
        GCList ls(gc);
        auto dummy =
            TNativeObject::Create<DummyTNativeObjectThatReturnsHttpDictionary>(
                ls);
...
bool TObjectHttpServer::Handle(ServerContext &ctx) {
    ...
    if (GetObjectHeap<TCallable *>(this->obj, callable)) {
        ...
        res->Finish();
    } else if (GetObjectHeap(this->obj, clsObj) &&
               clsObj->HasMethod("", "Handle")) {
        GCList ls2(this->ls->GetGC());
        auto res = TNativeObject::Create<TServerContext>(ls2, &ctx);
        bool result;
...
TObjectHttpServer::~TObjectHttpServer() { delete ls; }
...
class HttpServerNativeObject : public TNativeObject {
  ...
  public:
    HttpServerNativeObject(uint16_t port,
                           std::shared_ptr<IHttpServer> httpServer,
                           bool printIps) {
        server = new HttpServer(port, httpServer, printIps);
    }
    HttpServerNativeObject(std::string unixPath,
                           std::shared_ptr<IHttpServer> httpServer) {
        server = new HttpServer(unixPath, httpServer);
    }
    std::string TypeName() { return "Net.Http.HttpServer"; }
    TObject CallMethod(GCList &ls, std::string key, std::vector<TObject> args) {
        if (key == "getPort") {
    ...
    ~HttpServerNativeObject() { delete server; }
};

static TObject New_HttpServer(GCList &ls, std::vector<TObject> args,
    ...
    if (GetArgument(args, 1, port)) {
        ...
        std::shared_ptr<IHttpServer> httpSvr =
            ToHttpServer(ls.GetGC(), args[0]);

        if (httpSvr) {
            uint16_t p = (uint16_t)port;
            return TNativeObject::Create<HttpServerNativeObject>(
                ls, port, httpSvr, printIPs);
        }
    }
    ...
    if (GetArgument(args, 1, pathStr) && env->permissions.localfs) {
        ...
        if (httpSvr) {
            return TNativeObject::Create<HttpServerNativeObject>(ls, pathStr,
                                                                 httpSvr);
        }
    }
    return nullptr;
}
static TObject Net_Http_ListenSimpleWithLoop(GCList &ls,
...
static TObject Net_Http_ListenOnUnusedPort(GCList &ls,
                                           std::vector<TObject> args) {
    if (args.empty())
        return nullptr;
    std::shared_ptr<IHttpServer> httpSvr =
        ToHttpServer(ls.GetGC(), args.front());
    if (httpSvr) {
        uint16_t p = 0;
        HttpServer server(p, httpSvr);
...
class THttpResponse : public TNativeObjectThatReturnsHttpDictionary {
  ...
  public:
    THttpResponse(HttpResponse *resp) { this->response = resp; }
    bool IsAvailable() { return this->response != nullptr; }
    ...
    std::string TypeName() { return "Net.Http.HttpResponse"; }
    ...
    TObject CallMethod(GCList &ls, std::string key, std::vector<TObject> args) {
        if (!this->IsAvailable())
            ...
            return TNativeObject::Create<THttpDictionary>(
                ls, &response->responseHeaders, this);
        }
...
static TObject Net_Smtp_Send(GCList &ls, std::vector<TObject> args) {
    ...
    if (GetArgumentHeap(args, 0, dict)) {
        ...
        // check function return type just in case (this function returns a
        // empty string if no error) we rolled our own smtp client
        ...
        */
        ...
        if (GetObjectHeap(server, dict2)) {
            ...
            if (!GetObject(portO, port))
                port = tls ? 465 : 25;

            GetObject(hostO, host);
            strm = std::make_shared<NetworkStream>(host, (uint16_t)port, false,
                                                   false, false);
            if (tls) {
                strm = std::make_shared<Framework::Crypto::ClientTLSStream>(
                    strm, true, host);
            }
        ...
        if (GetObjectHeap(o, als)) {
            for (int64_t i = 0; i < als->Count(); i++) {
                ...
                if (GetObjectHeap(item, dict2)) {
                    ...
                    client.attachments.push_back(
                        std::pair<std::string, std::shared_ptr<SMTPBody>>(
                            name, TObjectToSMTPBody(ls, type, o)));
                }
            }
        }
...
static TObject Net_Http_WebSocketClient(GCList &ls, std::vector<TObject> args) {
    ...
    if (GetArgument(args, 0, url) && GetArgumentHeap(args, 1, headers) &&
        ...
        auto conn = std::make_shared<CallbackWebSocketConnection>(
            [dict, &ls](std::function<void(WebSocketMessage &)> sendMessage,
                        std::function<void()> ping,
                        std::function<void()> close) -> void {
                GCList ls2(ls.GetGC());
                dict->CallMethod(
                    ls2, "Open",
                    {TExternalMethod::Create(
                         ls2, "Send a message", {"messageTextOrByteArray"},
                         [sendMessage](GCList &ls,
                                       std::vector<TObject> args) -> TObject {
                             std::string str;
                             TByteArray *bArray;
                             if (GetArgument(args, 0, str)) {
        ...
        WebSocketClient(
            url, hdict, conn,
            [&ls, callable](Tesses::Framework::Http::HttpDictionary &dict,
                            bool success) -> bool {
                if (callable != nullptr) {
                    auto dummy = TNativeObject::Create<
                        DummyTNativeObjectThatReturnsHttpDictionary>(ls);
                    bool res = ToBool(callable->Call(
                        ls, {TNativeObject::Create<THttpDictionary>(ls, &dict,
                                                                    dummy),
                             success}));
                    dummy->Close();
                    return res;
        ...
        auto conn = std::make_shared<CallbackWebSocketConnection>(
            [co, &ls](std::function<void(WebSocketMessage &)> sendMessage,
                      std::function<void()> ping,
                      std::function<void()> close) -> void {
                GCList ls2(ls.GetGC());
                co->CallMethod(
        ...
        WebSocketClient(
            url, hdict, conn,
            [&ls, co](Tesses::Framework::Http::HttpDictionary &dict,
                      bool success) -> bool {
                if (co->HasMethod("", "HandleResponseHeaders")) {
                    auto dummy = TNativeObject::Create<
                        DummyTNativeObjectThatReturnsHttpDictionary>(ls);
                    bool res = ToBool(
                        co->CallMethod(ls, "", "HandleResponseHeaders",
                                       {TNativeObject::Create<THttpDictionary>(
                                            ls, &dict, dummy),
                                        success}));
                    dummy->Close();
                    return res;
    ...
    return Undefined();
    // Net.Http.CreateWebSocketConnection("wss://example.com/",[],conn, (dict,
    // success)=>{ return true;})
}
static TObject Net_Http_DownloadToString(GCList &ls,
                                         std::vector<TObject> args) {
    std::string url;
    if (GetArgument(args, 0, url))
...
static TObject Net_Http_DownloadToStream(GCList &ls,
                                         std::vector<TObject> args) {
    std::string url;
    std::shared_ptr<Stream> strm;
...
static TObject Net_Http_DownloadToFile(GCList &ls, std::vector<TObject> args) {
    ...
    if (GetArgument(args, 0, url) && GetArgument(args, 1, vfs) &&
        GetArgumentAsPath(args, 2, path)) {
        DownloadToFileSimple(url, vfs, path);
    }
    return nullptr;
}
bool IHttpServer_Handle(std::shared_ptr<IHttpServer> svr,
                        std::vector<TObject> &args) {
    TServerContext *ctx;
...
std::shared_ptr<IHttpServer> ToHttpServer(std::shared_ptr<GC> gc, TObject obj) {
    if (std::holds_alternative<std::shared_ptr<IHttpServer>>(obj))
        return std::get<std::shared_ptr<IHttpServer>>(obj);
    TDictionary *dict;
    TClassObject *clo;
    TCallable *call;
...
static TObject Net_Http_StatusCodeString(GCList &ls,
                                         std::vector<TObject> args) {
    int64_t sc;
    if (GetArgument(args, 0, sc)) {
...
static TObject New_FileServer(GCList &ls, std::vector<TObject> args) {
    ...
    bool allowlisting;
    bool spa;
    if (GetArgument(args, 0, vfs) && GetArgument(args, 1, allowlisting) &&
        GetArgument(args, 2, spa)) {
...
static TObject New_BasicAuthServer(GCList &ls, std::vector<TObject> args) {
    ...
    if (GetArgumentHeap(args, 1, cb)) {
        ...
        bAuth->authorization = [marked](std::string user,
...
static TObject New_MountableServer(GCList &ls, std::vector<TObject> args) {
    if (args.empty())
        return std::make_shared<MountableServer>();
...
static TObject New_RouteServer(GCList &ls, std::vector<TObject> args) {
    if (args.empty())
        return std::make_shared<RouteServer>();
    return std::make_shared<RouteServer>(ToHttpServer(ls.GetGC(), args[0]));
}
static TObject New_StreamHttpRequestBody(GCList &ls,
                                         std::vector<TObject> args) {
    std::shared_ptr<Stream> strm;
    std::string mimeType;
...
void TStd::RegisterNet(std::shared_ptr<GC> gc, TRootEnvironment *env) {
    ...
    _new->DeclareFunction(gc, "StreamHttpRequestBody",
                          "Create a stream request body",
                          {"stream", "mimeType"}, New_StreamHttpRequestBody);
    _new->DeclareFunction(gc, "TextHttpRequestBody",
    ...
    _new->DeclareFunction(
        ...
        {"server", "portOrUnixPath", "$printIPs"},
        [env](GCList &ls, std::vector<TObject> args) -> TObject {
            return New_HttpServer(ls, args, env);
        });

    _new->DeclareFunction(gc, "RouteServer", "Create a routeserver", {"$root"},
                          New_RouteServer);
    _new->DeclareFunction(gc, "FileServer", "Create a file server",
                          {"vfs", "allowlisting", "spa"}, New_FileServer);
    _new->DeclareFunction(gc, "BasicAuthServer", "Create a basic auth server",
    ...
    _new->DeclareFunction(gc, "NetworkStream", "Create a network stream",
    ...
    http->DeclareFunction(gc, "StatusCodeString", "Get the status code string",
                          {"statusCode"}, Net_Http_StatusCodeString);
    http->DeclareFunction(gc, "HtmlP", "Linkify text", {"text"},
                          Net_Http_HtmlP);
    http->DeclareFunction(gc, "HtmlEncode", "Html encode", {"param"},
                          Net_HtmlEncode);

    http->DeclareFunction(gc, "UrlEncode", "Url encode query param", {"param"},
                          Net_UrlEncode);
    http->DeclareFunction(gc, "UrlDecode", "Url decode query param", {"param"},
                          Net_UrlDecode);
    http->DeclareFunction(gc, "UrlPathEncode", "Url encode path", {"path"},
                          Net_UrlPathEncode);
    http->DeclareFunction(gc, "UrlPathDecode", "Url decode path", {"path"},
                          Net_UrlPathDecode);
    http->DeclareFunction(gc, "MimeType", "Get mimetype from extension",
                          {"ext"}, Net_Http_MimeType);

    http->DeclareFunction(gc, "StreamHttpRequestBody",
                          "Create a stream request body",
                          {"stream", "mimeType"}, New_StreamHttpRequestBody);
    http->DeclareFunction(gc, "TextHttpRequestBody",
                          "Create a text request body", {"text", "mimeType"},
                          New_TextHttpRequestBody);
    http->DeclareFunction(gc, "JsonHttpRequestBody",
                          "Create a text request body", {"json"},
                          New_JsonHttpRequestBody);
    http->DeclareFunction(
        gc, "MakeRequest", "Create an http request", {"url", "$extra"},
        [env](GCList &ls, std::vector<TObject> args) -> TObject {
            return Net_Http_MakeRequest(ls, args, env);
        });
    http->DeclareFunction(
        gc, "WebSocketClient", "Create a websocket connection",
        {"url", "headers", "conn", "$successCB"}, Net_Http_WebSocketClient);
    http->DeclareFunction(gc, "DownloadToString",
                          "Return the http file's contents as a string",
                          {"url"}, Net_Http_DownloadToString);
    http->DeclareFunction(gc, "DownloadToStream", "Download file to stream",
                          {"url", "stream"}, Net_Http_DownloadToStream);
    http->DeclareFunction(gc, "DownloadToFile", "Download file to file in vfs",
                          {"url", "vfs", "path"}, Net_Http_DownloadToFile);
    http->DeclareFunction(
        gc, "HttpServer", "Create a http server (allows multiple)",
        {"server", "portOrUnixPath", "$printIPs"},
        [env](GCList &ls, std::vector<TObject> args) -> TObject {
    ...
    http->DeclareFunction(gc, "ListenSimpleWithLoop",
                          "Listen (creates application loop)",
                          {"server", "port"}, Net_Http_ListenSimpleWithLoop);
    http->DeclareFunction(
        gc, "ListenOnUnusedPort",
        "Listen on unused localhost port and print Port: theport", {"server"},
        Net_Http_ListenOnUnusedPort);
    // FileServer svr()

    http->DeclareFunction(gc, "RouteServer", "Create a routeserver", {"$root"},
                          New_RouteServer);
    http->DeclareFunction(gc, "FileServer", "Create a file server",
                          {"vfs", "allowlisting", "spa"}, New_FileServer);
    http->DeclareFunction(gc, "BasicAuthServer", "Create a basic auth server",
                          {"$server", "$auth", "$realm"}, New_BasicAuthServer);
    http->DeclareFunction(
        gc, "BasicAuthGetCreds", "Get creds from str", {"ctx"},
        [](GCList &ls, std::vector<TObject> args) -> TObject {
            ...
            if (GetArgumentHeap(args, 0, sc)) {
                ...
                std::string pass;
                if (sc->IsAvailable() &&
                    BasicAuthServer::GetCreds(*sc->GetContext(), user, pass)) {
                    return TDictionary::Create(ls, {TDItem("Username", user),
    ...
    dict->DeclareFunction(
        ...
        [](GCList &ls, std::vector<TObject> args) -> TObject {
            ...
            ls.GetGC()->BarrierBegin();
            for (auto item :
                 Tesses::Framework::Streams::NetworkStream::GetIPs(ipv6)) {
                a->Add(
                    TDictionary::Create(ls, {TDItem("Interface", item.first),
...
TCallable::ToRouteServerRequestHandler(std::shared_ptr<GC> gc) {
