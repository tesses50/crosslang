...
template <typename T> class ffi_my_struct_t {
    ...
    if (std::holds_alternative<char>(arg)) {
        ...
        if (t == "u8" || t == "u16" || t == "u32") {
            return (ffi_tmp_type_t){
                .v_arg = (ffi_arg)(uint64_t)std::get<int64_t>(arg)};
...
TExternalMethod *FFI_CreateFunction(GCList &ls, TNative *native,
                                    std::string retVal,
                                    std::vector<std::string> listArgs,
                                    ffi_abi abi, bool freeRet) {
    ...
    auto cb = TExternalMethod::Create(
        ls, "FFI Generated function, returns: " + retVal, listArgs,
        [retVal, listArgs, abi, native, freeRet, argTypes,
         retTypeT](GCList &ls, std::vector<TObject> args) -> TObject {
            if (args.size() != listArgs.size())
                throw VMException("Args must be " +
                                  std::to_string(listArgs.size()) + "but got " +
            ...
            for (size_t i = 0; i < argTypes.size(); i++) {
                ...
                if (listArgs[i] == "char" || listArgs[i] == "i8" ||
                    listArgs[i] == "i16" || listArgs[i] == "i32" ||
                    listArgs[i] == "u8" || listArgs[i] == "u16" ||
                    listArgs[i] == "u32") {
            ...
            ffi_prep_cif(&cif, abi, (unsigned int)listArgs.size(), retTypeT,
                         (ffi_type **)argTypes.data());

            ffi_tmp_type_t retObj;
            void *retPtr = NULL;
            if (retVal == "char" || retVal == "i8" || retVal == "u8" ||
                retVal == "i16" || retVal == "u16" || retVal == "i32" ||
                retVal == "u32") {
            ...
            ffi_call(&cif, (void (*)())native->GetPointer(), retPtr,
                     argVals.data());

            if (retVal == "char")
                return (char)retObj.v_sarg;
            if (retVal == "i8" || retVal == "i16" || retVal == "i32")
                return (int64_t)retObj.v_sarg;
            if (retVal == "u8" || retVal == "u16" || retVal == "u32")
                return (int64_t)retObj.v_arg;
            if (retVal == "i64" || retVal == "u64")
                return retObj.v_i64;
            if (retVal == "f64")
                return retObj.v_f64;
            if (retVal == "string") {
            ...
            if (retVal == "pointer") {
                TNative *native3 = TNative::Create(
                    ls, retObj.v_ptr, [freeRet](void *_ptr) -> void {
                        if (freeRet)
                            free(_ptr);
                    });
...
void RegisterFFI(std::shared_ptr<GC> gc, TDictionary *dict) {
    ...
    dict->SetValue("PaddingOfLongLong",
                   ffi_my_struct_t<long long>::GetPadding());
    ...
    dict->DeclareFunction(
        gc, "Open", "Open a shared object", {"path"},
        [](GCList &ls, std::vector<TObject> args) -> TObject {
            Tesses::Framework::Filesystem::VFSPath path;
            if (GetArgumentAsPath(args, 0, path)) {
                return TNative::Create(
                    ls, static_cast<void *>(new DL(path)),
                    [](void *ptr) -> void { delete static_cast<DL *>(ptr); });
            }
            return nullptr;
        });
    dict->DeclareFunction(
        gc, "Symbol", "Get current symbol", {"sharedObj", "name"},
        [](GCList &ls, std::vector<TObject> args) -> TObject {
            TNative *native;
            std::string name;
            if (GetArgumentHeap(args, 0, native) &&
                GetArgument(args, 1, name)) {
                if (native->GetDestroyed())
                    return nullptr;
                DL *dl = static_cast<DL *>(native->GetPointer());
                auto native2 = TNative::Create(ls, dl->Resolve<void *>(name),
                                               [](void *_e) -> void {});
    ...
    dict->DeclareFunction(gc, "Close", "Closes the shared object",
                          {"sharedObj"},
                          [](GCList &ls, std::vector<TObject> args) -> TObject {
                              TNative *native;
                              if (GetArgumentHeap(args, 0, native))
    ...
    dict->DeclareFunction(
        gc, "CreateFunction", "Create a function",
        {"functionPtr", "returnVal", "args", "$abi", "$freeRet"},
        [](GCList &ls, std::vector<TObject> args) -> TObject {
            TNative *native;
            std::string retVal;
            TList *listArgsO;
            ...
            bool freeRet = false;
            if (GetArgumentHeap(args, 0, native) &&
                GetArgument(args, 1, retVal) &&
                GetArgumentHeap(args, 2, listArgsO)) {
                ...
                for (int64_t i = 0; i < listArgsO->Count(); i++) {
                    ...
                    std::string typ;
                    if (GetObject(item, typ))
                        listArgs.push_back(typ);
                }

                return FFI_CreateFunction(ls, native, retVal, listArgs,
                                          (ffi_abi)abi, freeRet);
            }
            return nullptr;
        });
...
GetPluginPath(Tesses::Framework::Filesystem::VFSPath path) {
    ...
    if (LocalFS->DirectoryExists(pluginConfigDir)) {
        ...
        auto path2 = pluginConfigDir / path.GetParent() / "lib" +
                     path.GetFileName() + SharedExtension;
        if (LocalFS->FileExists(path2))
            return path2;
    }
...
void LoadPlugin(std::shared_ptr<GC> gc, TRootEnvironment *env,
    ...
    if (cb == nullptr)
        return;
    gc->RegisterEverythingCallback(
        [ptr, cb](std::shared_ptr<GC> gc, TRootEnvironment *env) -> void {
            cb(gc, env);
        });
    cb(gc, env);
#endif
...
static TObject New_SubdirFilesystem(GCList &ls, std::vector<TObject> args) {
    ...
    if (GetArgument(args, 0, vfs) && GetArgumentAsPath(args, 1, path)) {
        return std::make_shared<
            Tesses::Framework::Filesystem::SubdirFilesystem>(vfs, path);
    }
    return nullptr;
}
...
static TObject New_MemoryStream(GCList &ls, std::vector<TObject> args) {
    ...
    if (GetArgument(args, 0, writable)) {
        return std::make_shared<Tesses::Framework::Streams::MemoryStream>(
            writable);
    }
    return nullptr;
}
        ...
        if (args.size() == 1) {
            ...
            if (GetArgument(args, 1, arg2) && GetArgument(args, 2, arg3)) {
                ...
                if (GetArgument(args, 3, arg4)) {
                    return std::make_shared<Tesses::Framework::Date::TimeSpan>(
...
std::string GetObjectTypeString(TObject _obj) {
    if (std::holds_alternative<
            std::shared_ptr<Tesses::Framework::TF_Timer_Handle>>(_obj))
        return "Timer";

    if (std::holds_alternative<
            std::shared_ptr<Tesses::Framework::Http::ServerSentEvents>>(_obj))
        return "ServerSentEvents";
    if (std::holds_alternative<std::regex>(_obj))
        return "Regex";
    ...
    if (std::holds_alternative<bool>(_obj))
        return "Boolean";
    if (std::holds_alternative<int64_t>(_obj))
        return "Long";
    if (std::holds_alternative<double>(_obj))
        return "Double";
    if (std::holds_alternative<char>(_obj))
        return "Char";
    if (std::holds_alternative<MethodInvoker>(_obj))
        return "MethodInvoker";
    if (std::holds_alternative<std::string>(_obj))
        return "String";

    if (std::holds_alternative<Tesses::Framework::Filesystem::VFSPath>(_obj))
        return "Path";
    if (std::holds_alternative<TVMVersion>(_obj))
        return "Version";
    if (std::holds_alternative<
            std::shared_ptr<Tesses::Framework::Date::DateTime>>(_obj))
        return "DateTime";
    if (std::holds_alternative<
            std::shared_ptr<Tesses::Framework::Date::TimeSpan>>(_obj))
        return "TimeSpan";
    if (std::holds_alternative<
            std::shared_ptr<Tesses::Framework::Streams::ByteReader>>(_obj))
        return "ByteReader";

    if (std::holds_alternative<
            std::shared_ptr<Tesses::Framework::Streams::ByteWriter>>(_obj))
        return "ByteWriter";

    if (std::holds_alternative<Tesses::Framework::Uuid>(_obj))
        return "Uuid";
    if (std::holds_alternative<
            std::shared_ptr<Tesses::Framework::Streams::Stream>>(_obj)) {
        auto strm =
            std::get<std::shared_ptr<Tesses::Framework::Streams::Stream>>(_obj);
        if (strm != nullptr) {
    ...
    if (std::holds_alternative<
            std::shared_ptr<Tesses::Framework::Http::IHttpServer>>(_obj)) {
        auto svr =
            std::get<std::shared_ptr<Tesses::Framework::Http::IHttpServer>>(
                _obj);
        if (svr != nullptr) {
            auto fileServer =
                std::dynamic_pointer_cast<Tesses::Framework::Http::FileServer>(
                    svr);
            auto mountableServer = std::dynamic_pointer_cast<
                Tesses::Framework::Http::MountableServer>(svr);
            auto routableServer =
                std::dynamic_pointer_cast<Tesses::Framework::Http::RouteServer>(
                    svr);
    ...
    if (std::holds_alternative<
        ...
        auto textReader = std::get<
            std::shared_ptr<Tesses::Framework::TextStreams::TextReader>>(_obj);
        if (textReader != nullptr) {
            auto stringReader = std::dynamic_pointer_cast<
                Tesses::Framework::TextStreams::StringReader>(textReader);
            auto streamReader = std::dynamic_pointer_cast<
                Tesses::Framework::TextStreams::StreamReader>(textReader);
            auto consoleReader = std::dynamic_pointer_cast<
                Tesses::Framework::TextStreams::ConsoleReader>(textReader);
    ...
    if (std::holds_alternative<
        ...
        auto textWriter = std::get<
            std::shared_ptr<Tesses::Framework::TextStreams::TextWriter>>(_obj);
        if (textWriter != nullptr) {
            auto stringWriter = std::dynamic_pointer_cast<
                Tesses::Framework::TextStreams::StringWriter>(textWriter);
            auto streamWriter = std::dynamic_pointer_cast<
                Tesses::Framework::TextStreams::StreamWriter>(textWriter);
            auto consoleWriter = std::dynamic_pointer_cast<
                Tesses::Framework::TextStreams::ConsoleReader>(textWriter);
    ...
    if (std::holds_alternative<
            std::shared_ptr<Tesses::Framework::Filesystem::VFS>>(_obj)) {
        auto vfs =
            std::get<std::shared_ptr<Tesses::Framework::Filesystem::VFS>>(_obj);
        if (vfs != nullptr) {
            auto rfs = std::dynamic_pointer_cast<
                Tesses::Framework::Filesystem::RelativeFilesystem>(vfs);
            auto localVFS = std::dynamic_pointer_cast<
                Tesses::Framework::Filesystem::LocalFilesystem>(vfs);
            auto mountableVFS = std::dynamic_pointer_cast<
                Tesses::Framework::Filesystem::MountableFilesystem>(vfs);
            auto subFS = std::dynamic_pointer_cast<
                Tesses::Framework::Filesystem::SubdirFilesystem>(vfs);
            auto tempFS = std::dynamic_pointer_cast<
            ...
            if (subFS != nullptr)
                return "SubdirFilesystem";
            if (mountableVFS != nullptr)
    ...
    if (std::holds_alternative<THeapObjectHolder>(_obj)) {
        ...
        if (env != nullptr)
            return "Environment";
        if (cobj != nullptr)
            return cobj->TypeName();

        if (cse != nullptr)
            return "YieldedClosure";
        if (dynDict != nullptr)
        ...
        if (natObj != nullptr)
            return natObj->TypeName();

        if (dict != nullptr)
            return "Dictionary";
        if (list != nullptr)
            return "List";
        if (argWrapper != nullptr)
            return "ArgWrapper";
        if (closure != nullptr)
            return "Closure";
...
static TObject YieldEnumerableFunc(GCList &ls, std::vector<TObject> args) {
    ...
    if (GetArgumentHeap(args, 0, closure)) {
        ...
        auto fn = TExternalMethod::Create(
            ls, "Get Enumerator for yield", {},
            [closure](GCList &ls2, std::vector<TObject> args) -> TObject {
...
static TObject New_Promise(GCList &ls, std::vector<TObject> args) {
    ...
    if (GetArgumentHeap(args, 0, call)) {
        ...
        TExternalMethod *resolve = TExternalMethod::Create(
            ls, "fulfill the promise", {"arg"},
            [task](GCList &ls, std::vector<TObject> args) -> TObject {
                if (!args.empty()) {
        ...
        TExternalMethod *reject = TExternalMethod::Create(
            ls, "reject the promise", {"arg"},
            [task](GCList &ls, std::vector<TObject> args) -> TObject {
                if (!args.empty()) {
                    auto item = args[0];
...
static TObject TimeSpan_Parse(GCList &ls, std::vector<TObject> args) {
    ...
    if (GetArgument(args, 0, t) &&
        Tesses::Framework::Date::TimeSpan::TryParse(t, ts)) {
...
static TObject New_StreamReader(GCList &ls, std::vector<TObject> args) {
    ...
    if (GetArgument(args, 0, strm)) {
        return std::make_shared<Tesses::Framework::TextStreams::StreamReader>(
            strm);
...
static TObject New_StreamWriter(GCList &ls, std::vector<TObject> args) {
    ...
    if (GetArgument(args, 0, strm)) {
        return std::make_shared<Tesses::Framework::TextStreams::StreamWriter>(
            strm);
    } else if (GetArgument(args, 0, vfs) && GetArgumentAsPath(args, 1, path)) {
        bool append = false;
...
static TObject New_StringReader(GCList &ls, std::vector<TObject> args) {
    ...
    if (GetArgument(args, 0, str)) {
        return std::make_shared<Tesses::Framework::TextStreams::StringReader>(
            str);
    }
    return nullptr;
}
...
static TObject New_Timer(GCList &ls, std::vector<TObject> args) {
    ...
    if (GetArgumentHeap(args, 0, callable)) {
        ...
        return Tesses::Framework::TF_Timer(
            [obj]() -> void {
                TCallable *callable;
...
static TObject New_RelativeFilesystem(GCList &ls, std::vector<TObject> args) {
    ...
    if (GetArgument(args, 0, vfs) && GetArgumentAsPath(args, 1, path) &&
        !path.relative) {
        return std::make_shared<
            Tesses::Framework::Filesystem::RelativeFilesystem>(vfs, path);
    }
    return nullptr;
}
...
void TStd::RegisterRoot(std::shared_ptr<GC> gc, TRootEnvironment *env) {
    ...
    date->DeclareFunction(gc, "Sleep",
                          "Sleep for a specified amount of milliseconds "
                          "(multiply seconds by 1000 to get milliseconds)",
                          {"ms"}, DateTime_Sleep);
    date->DeclareFunction(gc, "getNow", "Get the current time", {},
                          DateTime_getNow);
    date->DeclareFunction(gc, "getNowUTC", "Get the current time in UTC", {},
                          DateTime_getNowUTC);
    date->DeclareFunction(gc, "getNowEpoch", "Get the time_t time now", {},
                          DateTime_getNowEpoch);
    date->DeclareFunction(gc, "TryParseHttpDate", "Parse the http date", {},
                          DateTime_TryParseHttpDate);

    date->SetValue("Zone", (int64_t)Tesses::Framework::Date::GetTimeZone());
    date->SetValue("SupportsDaylightSavings",
                   Tesses::Framework::Date::TimeZoneSupportDST());

    auto ts = env->EnsureDictionary(gc, "TimeSpan");
    ts->DeclareFunction(gc, "Parse", "Parse timespan", {"tsStr"},
                        TimeSpan_Parse);
    ts->DeclareFunction(gc, "FromSeconds", "Create timespan from seconds",
                        {"seconds"}, TimeSpan_FromSeconds);
    ts->DeclareFunction(gc, "FromMinutes", "Create timespan from minutes",
                        {"minutes"}, TimeSpan_FromMinutes);
    ts->DeclareFunction(gc, "FromHours", "Create timespan from hours",
                        {"hours"}, TimeSpan_FromHours);
    ts->DeclareFunction(gc, "FromDays", "Create timespan from days", {"days"},
                        TimeSpan_FromDays);

    auto task = env->EnsureDictionary(gc, "Task");

    task->DeclareFunction(
        gc, "AsyncClosure",
        "Create async closure (internal for compiler to generate calls to)",
        {"closure"}, [](GCList &ls, std::vector<TObject> args) -> TObject {
            TClosure *closure;
            if (GetArgumentHeap(args, 0, closure))
    ...
    task->DeclareFunction(gc, "Run", "Run code async", {"callable"},
                          [](GCList &ls, std::vector<TObject> args) -> TObject {
                              TCallable *closure;
                              if (GetArgumentHeap(args, 0, closure))
    ...
    task->DeclareFunction(gc, "FromResult", "async from result", {"result"},
                          Task_FromResult);

    TDictionary *newTypes = env->EnsureDictionary(gc, "New");

    newTypes->DeclareFunction(gc, "ByteReader", "Read binary data from stream",
                              {"stream"}, New_ByteReader);

    newTypes->DeclareFunction(gc, "ByteWriter", "Write binary data to stream",
                              {"stream"}, New_ByteWriter);
    // newTypes->DeclareFunction(gc,)
    newTypes->DeclareFunction(gc, "Promise", "Create an async object",
                              {"resolve", "reject"}, New_Promise);
    newTypes->DeclareFunction(
        gc, "DateTime",
        "Create a DateTime object, if only one arg is provided year is epoch, "
        "isLocal defaults to true unless epoch",
        {"year", "$month", "$day", "$hour", "$minute", "$second", "$isLocal"},
        New_DateTime);
    newTypes->DeclareFunction(
        gc, "TimeSpan",
        "Create a DateTime object, if only one arg is provided days is "
        "totalSeconds, if there are only three arguments days will be hours, "
        "hours will be minutes, minutes will be seconds (according to the "
        "argument documentation)",
        {"days", "$hours", "$minutes", "$seconds"}, New_TimeSpan);

    newTypes->DeclareFunction(gc, "MountableFilesystem",
                              "Create a mountable filesystem", {"root"},
                              New_MountableFilesystem);
    newTypes->DeclareFunction(gc, "SubdirFilesystem",
                              "Create a subdir filesystem", {"fs", "subdir"},
                              New_SubdirFilesystem);
    newTypes->DeclareFunction(gc, "MemoryStream", "Create a memory stream",
                              {"writable"}, New_MemoryStream);
    newTypes->DeclareFunction(gc, "Filesystem", "Create filesystem", {"fs"},
                              New_Filesystem);
    newTypes->DeclareFunction(gc, "RelativeFilesystem", "Create relativefs",
                              {"parent", "absoluteDir"},
                              New_RelativeFilesystem);
    newTypes->DeclareFunction(gc, "TempFS", "Create a temp directory", {"", ""},
                              New_TempFS);
    newTypes->DeclareFunction(gc, "Timer", "Create a timer",
                              {"$cb", "$interval", "$enabled"}, New_Timer);
    newTypes->DeclareFunction(gc, "Stream", "Create stream", {"strm"},
                              New_Stream);

    newTypes->DeclareFunction(
        gc, "Version", "Create a version object",
        {"$major", "$minor", "$patch", "$build", "$stage"},
        [](GCList &ls, std::vector<TObject> args) -> TObject {
            int64_t major = 1;
            int64_t minor = 0;
            int64_t patch = 0;
            ...
            return TVMVersion((uint8_t)major, (uint8_t)minor, (uint8_t)patch,
                              (uint16_t)build, stage);
        });
    newTypes->DeclareFunction(
        gc, "Random", "Create random number generator", {"$seed"},
        [](GCList &ls, std::vector<TObject> args) -> TObject {
            int64_t seed;
    ...
    newTypes->DeclareFunction(gc, "StreamReader", "Create a StreamReader",
                              {"strmOrVFS", "$pathIfVFS"}, New_StreamReader);
    newTypes->DeclareFunction(gc, "StreamWriter", "Create a StreamWriter",
                              {"strmOrVFS", "$pathIfVFS", "$appendIfVFS"},
                              New_StreamWriter);
    newTypes->DeclareFunction(gc, "StringReader", "Create a StringReader",
                              {"str"}, New_StringReader);
    newTypes->DeclareFunction(gc, "StringWriter", "Create a StringWriter",
                              {"$str"}, New_StringWriter);

    env->DeclareFunction(gc, "ParseLong", "Parse Long from String",
                         {"arg", "$base"}, ParseLong);
    env->DeclareFunction(gc, "ParseDouble", "Parse Double from String", {"arg"},
                         ParseDouble);
    env->DeclareFunction(gc, "YieldEmumerable",
                         "Turn yield in function into enumerable", {"closure"},
                         YieldEnumerableFunc);
    env->DeclareFunction(gc, "TypeOf", "Get type of object", {"object"},
                         TypeOf);
    ...
    env->DeclareFunction(gc, "TypeIsLong",
                         ...
                         {"object"}, TypeIsLong);
    env->DeclareFunction(gc, "TypeIsDouble",
                         "Get whether object is a double (not a long)",
                         {"object"}, TypeIsDouble);
    env->DeclareFunction(gc, "TypeIsString", "Get whether object is a string",
                         {"object"}, TypeIsString);
    env->DeclareFunction(gc, "TypeIsCallable", "Get whether object is callable",
                         {"object"}, TypeIsCallable);

    env->DeclareFunction(gc, "TypeIsClass",
                         "Get whether object is class instance", {"object"},
                         TypeIsClass);
    env->DeclareFunction(
        gc, "TypeIsDictionary",
        "Get whether object is a dictionary or dynamic dictionary", {"object"},
        TypeIsDictionary);
    env->DeclareFunction(gc, "TypeIsList",
                         "Get whether object is a list or dynamic list",
                         {"object"}, TypeIsList);
    env->DeclareFunction(gc, "TypeIsStream", "Get whether object is a stream",
                         {"object"}, TypeIsStream);
    env->DeclareFunction(gc, "TypeIsVFS",
                         "Get whether object is a virtual filesystem",
                         {"object"}, TypeIsVFS);
    env->DeclareFunction(gc, "TypeIsDateTime",
                         "Get whether object is a DateTime", {"object"},
                         TypeIsDateTime);
    env->DeclareFunction(gc, "TypeIsTimeSpan",
                         "Get whether object is a TimeSpan", {"object"},
                         TypeIsTimeSpan);
    env->DeclareFunction(gc, "TypeIsTextReader",
                         "Get whether object is a TextReader", {"object"},
                         TypeIsTextReader);
    env->DeclareFunction(gc, "TypeIsTextWriter",
                         "Get whether object is a TextWriter", {"object"},
                         TypeIsTextWriter);
    env->DeclareFunction(gc, "TypeIsByteReader",
                         "Get whether object is a ByteReader", {"object"},
                         TypeIsByteReader);
    env->DeclareFunction(gc, "TypeIsByteWriter",
                         "Get whether object is a ByteWriter", {"object"},
                         TypeIsByteWriter);
    env->DeclareFunction(gc, "TypeIsUuid", "Get whether object is a Uuid",
                         {"object"}, TypeIsUuid);
    ...
    newTypes->DeclareFunction(
        gc, "DynamicList", "Create a dynamic list", {},
        [](GCList &ls, std::vector<TObject> args) -> TObject {
            TCallable *callable;
            if (GetArgumentHeap(args, 0, callable))
    ...
    newTypes->DeclareFunction(
        gc, "DynamicDictionary", "Create a dynamic dictionary", {},
        [](GCList &ls, std::vector<TObject> args) -> TObject {
            TCallable *callable;
            if (GetArgumentHeap(args, 0, callable))
    ...
    newTypes->DeclareFunction(
        gc, "AssociativeArray", "Create a new AssociativeArray", {},
        [](GCList &ls, std::vector<TObject> args) -> TObject {
            return TAssociativeArray::Create(ls);
        });
    newTypes->DeclareFunction(
        gc, "AArray", "alias for new AssociativeArray", {},
        [](GCList &ls, std::vector<TObject> args) -> TObject {
            return TAssociativeArray::Create(ls);
        });
    newTypes->DeclareFunction(
        ...
        "argument (to fill byte array)",
        {"$data"}, ByteArray);
    newTypes->DeclareFunction(gc, "Queryable", "Create a queryable",
                              {"enumerable"}, New_Queryable);
        ...
        TDictionary::Create(
            ...
            {TDItem("Parse",
                    TExternalMethod::Create(
                        ls, "Parse version from string", {"versionStr"},
                        [](GCList &ls, std::vector<TObject> args) -> TObject {
                            std::string str;
                            TVMVersion v;
             ...
             TDItem("FromByteArray",
                    TExternalMethod::Create(
                        ls, "Create from ByteArray", {"byteArray", "$offset"},
                        [](GCList &ls, std::vector<TObject> args) -> TObject {
                            TByteArray *ba;
                            if (GetArgumentHeap(args, 0, ba)) {
                                ...
                                if (ba->data.size() < 5)
                                    throw VMException("ByteArray too small");
                                size_t o = (size_t)offset;
                                if ((o + 5) > ba->data.size() || (o + 5) < 5)
                                    throw VMException("ByteArray too small");
             ...
             TDItem("FromLong",
                    TExternalMethod::Create(
                        ls, "Create from long", {"longBasedVersion"},
                        [](GCList &ls, std::vector<TObject> args) -> TObject {
                            int64_t num;
...
void TStd::RegisterStd(
    ...
    dict->DeclareFunction(
        gc, "LoadNativePlugin",
        "Load a native plugin, requires a dynamic linker and shared build of "
        "libcrosslang",
        {"path"}, [gc, env](GCList &ls, std::vector<TObject> args) -> TObject {
            Tesses::Framework::Filesystem::VFSPath path;
            if (GetArgumentAsPath(args, 0, path)) {
    ...
    gc_dict->DeclareFunction(
        gc, "BarrierBegin", "Lock globally", {},
        [](GCList &ls, std::vector<TObject> args) -> TObject {
            ls.GetGC()->BarrierBegin();
            return nullptr;
        });
    gc_dict->DeclareFunction(
        gc, "BarrierEnd", "Unlock globally", {},
        [](GCList &ls, std::vector<TObject> args) -> TObject {
            ls.GetGC()->BarrierEnd();
            return nullptr;
        });
